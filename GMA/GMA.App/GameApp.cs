...
public class GameApp
{
            ...
            else
            {
                switch (choice.ToUpper())
                {
                    ...
                    case "S":
                        ...
                        break;
                    case "G":
                        int choiceId = ChoiceGenreId(currentPage, kw, gid);
    ...
    public static void DisplayListGameTable(List<Game> games, int startIndex, int endIndex, int pageSize)
    {
        ...
        {
            Game game = games[i];
            Game gameInCart = orderBLL.GetCartGames().Find(gameInCart => gameInCart.GameId == game.GameId);
            ...
            {
                bool gameOwned = accountBLL.GetAccountLoggedIn().AccountOrders
    ...
    public static int ChoiceGenreId(int currentPage, string kw, int gid)
    {
    ...
    public static void GameDetailsMenu(int id, int currentPage, string keywords, int genID)
    {
        ...
        Game gameInCart = orderBLL.GetCartGames().Find(game => game.GameId == id);

        bool gameOwned = false; string capString = "";
        if (accountBLL.GetAccountLoggedIn() != null && accountBLL.GetAccountLoggedIn().AccountOrders != null)
        {
            gameOwned = accountBLL.GetAccountLoggedIn().AccountOrders
                .Any(order => order.OrderGames.Any(gameOwned => gameOwned.GameId == game.GameId));
        }
